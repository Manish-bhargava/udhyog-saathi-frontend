...
const DashboardLayout = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex overflow-hidden">
      <aside className={`
        fixed inset-y-0 left-0 z-40 bg-gradient-to-b from-slate-900 to-slate-800 text-white transition-all duration-300 ease-in-out flex flex-col
      ...
      `}>
        <div className="p-6 border-b border-slate-700/50 shrink-0">
          <div className="flex items-center space-x-3">
            ...
            <div className="flex-1 min-w-0">
              <h2 className="text-xl font-bold tracking-tight">UdhyogSaathi</h2>
          ...
          {navItems.map((item) => (
            <div key={item.path}>
              <button
                ...
                className={`
                  w-full flex items-center rounded-xl transition-all duration-200 group px-4 py-3.5 space-x-3
        ...
        <div className="p-4 border-t border-slate-700/50 shrink-0 relative">
          <button
            onMouseEnter={() => setShowProfileMenu(true)}
            className="w-full flex items-center rounded-xl hover:bg-slate-800/50 transition-all p-3 space-x-3"
          >
          ...
          {showProfileMenu && (
            ...
            >
              <Link 
                to="/profile" 
                className="w-full flex items-center px-4 py-3.5 text-sm text-slate-300 hover:bg-slate-700/80 transition-colors"
              >
                <span className="mr-3">👤</span> Profile Settings
              </Link>
              <button 
                onClick={handleLogout}
                className="w-full flex items-center px-4 py-3.5 text-sm text-slate-300 hover:bg-red-600/80 transition-colors"
              >
                <span className="mr-3">🚪</span> Logout
              </button>
            </div>
      ...
      <div className="flex-1 flex flex-col transition-all duration-300">
        <header className="h-20 bg-white border-b border-gray-200 px-6 md:px-8 flex items-center justify-between sticky top-0 z-30 shadow-sm">
          ...
          <div className="flex items-center space-x-3">
            <div className="relative hidden lg:block">
              <input
                type="text"
                placeholder="Search resources, bills, clients..."
                className="pl-11 pr-4 py-3 w-80 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all text-sm outline-none"
              />
              <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 text-lg">🔍</span>
            </div>

            <button className="p-3 text-gray-500 hover:bg-gray-100 rounded-xl relative transition-colors">
              <span className="text-xl">🔔</span>
        ...
        <main className="flex-1 overflow-y-auto p-6 md:p-8">
          {showBanner && (
            <div className="bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 rounded-2xl p-6 mb-8 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              <div className="flex items-start space-x-4">
                <div className="bg-amber-100 p-3.5 rounded-xl">
              ...
              <button
                onClick={() => navigate('/profile')}
                className="bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white px-6 py-3 rounded-xl text-sm font-bold shadow-md transition-all whitespace-nowrap active:scale-95"
              >
                Complete Onboarding
              </button>
            </div>
