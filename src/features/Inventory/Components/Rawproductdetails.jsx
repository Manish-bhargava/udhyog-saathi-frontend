...
export default function Rawproductdetails({
  product,
  onDeleteSuccess,
  onUpdateSuccess,
  ...
  const handleDelete = async () => {
    ...
    try {
      await inventoryAPI.deleteRawItem(product.id);
    ...
    <>
      ...
      <div className="bg-white rounded-xl p-5 shadow-sm">
        ...
        {product.image ? (
          ...
          <div className="rounded-lg mb-4 w-full h-40 bg-gray-100 flex items-center justify-center text-gray-400 text-sm">
            No Image
          </div>
        )}
        ...
        <div className="grid grid-cols-2 gap-3 text-sm mt-4">
          ...
          {!product.warehouseName && product.location && (
            <p className="col-span-2"><b>Location:</b> {product.location}</p>
          )}
        </div>
