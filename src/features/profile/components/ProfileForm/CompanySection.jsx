...
const CompanySection = () => {
  const { profile } = useProfile(); // Removed isCompanyLocked
  ...
  // Check if company email exists (means it's already been saved)
  const isEmailLocked = !!profile.company?.companyEmail;
  const isFirstSave = !profile.company?.companyEmail;
  
  useEffect(() => {
    if (profile && profile.company && !isLoading) {
      setFormData({
        companyName: profile.company.companyName || '',
        GST: profile.company.GST || '',
        companyAddress: profile.company.companyAddress || '',
  ...
  const handleFormSubmit = async (e) => {
    ...
    // Show email confirmation only during onboarding (first save)
    if (isFirstSave && formData.companyEmail && !isEmailLocked) {
  ...
  const submitCompanyData = async () => {
    ...
    const result = await handleSubmit(null, submitData);
    return result;
  };
  ...
  return (
    <>
      {/* Email Confirmation Modal (only shown during onboarding) */}
        ...
        <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            <div className="flex items-center mb-4">
              ...
              <h3 className="ml-3 text-lg font-medium text-gray-900">Confirm Your Company Email</h3>
            </div>
            <div className="mt-2">
              <p className="text-sm text-gray-500">
                You are about to set your company email to: <strong>{formData.companyEmail}</strong>
              </p>
              <p className="text-sm text-gray-500 mt-2">
                <strong>Important:</strong> This email cannot be changed after saving. It will be used for all company communications and billing.
              </p>
              <p className="text-sm text-gray-500 mt-2">
            ...
            <div>
              ...
              {isEmailLocked && (
                <p className="mt-1 text-xs text-gray-500">
                  Company email cannot be changed once saved.
                </p>
              )}
            ...
            >
              {isLoading ? (
                ...
                <>
                  ...
                  {isFirstSave ? 'Complete Onboarding' : 'Update Company Details'}
                </>
              )}
            </button>
          </div>
