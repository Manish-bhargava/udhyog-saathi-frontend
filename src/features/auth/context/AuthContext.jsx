...
export const AuthProvider = ({ children }) => {
  ...
  const signup = async (userData) => {
    try {
      ...
      if (response.token || response.data?.token) {
        const userToSet = response.data?.data || response.data;
        setUser(userToSet); 
        setIsNewUser(true);
        setIsOnboarded(false);
        
        // Explicitly set these to ensure no race conditions
  ...
  const isAuthenticated = useCallback(() => {
    ...
    // FIX: You must define 'storedUser' inside this function scope
    const storedUser = localStorage.getItem('user');
    
    // Debug logging to help you track the sync
